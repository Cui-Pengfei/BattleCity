...
public class MyTool {
    ...
    public static void drawRecord(Graphics g, Record record) {
		...
		*/
        ...
        int interval = 30;
        y = y + 400;
        g.drawString("游戏规则如下：", x, y + interval);
        g.drawString("发射炮弹： 空格：", x, y + 2 * interval);
        g.drawString("移动：上下左右箭头", x, y + 3 * interval);
        g.drawString("敌军暂停： s", x, y + 4 * interval);
        g.drawString("解除暂停： b", x, y + 5 * interval);
        g.drawString("击败所有敌军为胜利", x, y + 6 * interval);
    ...
    public static boolean isSuccessHit(FireBall ball, Tank tank) {
        return Math.pow(ball.centerX() - tank.centerX(), 2)
